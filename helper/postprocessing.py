...
def write_vector_to_paraview2(filename, mesh, us):
    ...
    u = us[0].copy()
    with dolfinx.io.VTXWriter(mesh.comm, Path(filename).with_suffix(".bp"), [u], engine="BP4") as vtx:
...
def write_tensor_to_paraview(filename, mesh, tensors):
    ...
    tensor_element = basix.ufl.element(family="DG", cell=str(mesh.ufl_cell()), degree=0, shape=(3,3))
