...
def initiate_functions(mesh):
    ...
    tensor_space = dolfinx.fem.functionspace(mesh, basix.ufl.element(family="DG", cell=str(mesh.ufl_cell()), degree=1, shape=(3,3)))
...
def GEG(mesh, F):
    ...
    F_g_elem = recip_tau*pow((F_gmax - F_g_prev_function[0,0])/(F_gmax - 1), gamma)*(F_e_function[0,0] - lambda_crit) + F_g_prev_function[0,0]
    ...
    F_g_prev_expression = dolfinx.fem.Expression(F_g, tensor_space.element.interpolation_points())
    F_e_expression = dolfinx.fem.Expression(F_e, tensor_space.element.interpolation_points())
...
def GCG(mesh, F):
    ...
    gamma = 2.0
    p_crit = 0.12
    F_g_elem = recip_tau*pow((F_gmax - F_g_prev_function[1,1])/(F_gmax - 1), gamma)*(ufl.tr(mandel_stress_function) - p_crit) + F_g_prev_function[1,1]
...
def LnT(mesh, F):
    ...
    D = 1
    F_grr = F_g_prev_function[0,0]/(1 - D*F_g_prev_function[0,0]*sigma_function[0,0])
    F_gff = F_g_prev_function[1,1]/(1 - D*F_g_prev_function[1,1]*sigma_function[1,1])
    F_gcc = F_g_prev_function[2,2]/(1 - D*F_g_prev_function[2,2]*sigma_function[2,2])
...
def ART(mesh, F):
    ...
    '''Initiate first growth tensor'''
    tensor_space = dolfinx.fem.functionspace(mesh, basix.ufl.element(family="CG", cell=str(mesh.ufl_cell()), degree=1, shape=(3,3)))
    X = ufl.SpatialCoordinate(mesh)       # get Identity without it being a constant
    Identity = ufl.variable(ufl.grad(X)) 
    Identity_expression = dolfinx.fem.Expression(Identity, tensor_space.element.interpolation_points())

    F_g_tot_function = dolfinx.fem.Function(tensor_space); F_g_tot_function.interpolate(Identity_expression)
    F_e_function = dolfinx.fem.Function(tensor_space); #E_e_function.interpolate(Identity_expression)
    L_function = dolfinx.fem.Function(tensor_space); L_function.interpolate(Identity_expression)
    ...
    F_grr = ufl.sqrt((1+a)/1+a*ufl.exp(b*L_s_max_adapt - L_function*F_e[1,1]))
    F_gcc = F_gff
    ...
    L_expression = dolfinx.fem.Expression(L_function*F_e, tensor_space.element.interpolation_points())
