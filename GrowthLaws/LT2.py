...
F_g_prev_function, F_g_prev_expression, sigma_stress_function, F_e = gl.LT2(mesh, F)
J_e = ufl.det(F_e); F_e_bar = F_e*pow(J_e, -1/3); C_e_bar = F_e_bar.T*F_e_bar
...
psi_inc  = psi.neohookean(mu, C_e_bar)
...
sigma_stress_expression = dolfinx.fem.Expression(P*F_e.T/J_e, tensor_space.element.interpolation_points())
...
us = []; F_g_f_tot = []; F_g_c_tot = []; F_e_f = []; F_e_c = []
...
for i in range(0, N+1):
    ...
    sigma_stress_function.interpolate(sigma_stress_expression)                # Update elastic deformation tensor

    F_g_f_tot.append(ddf.eval_expression(F_g_prev_function[0,0], mesh)[0,0])
    F_g_c_tot.append(ddf.eval_expression(F_g_prev_function[1,1], mesh)[0,0])
...
with open("../Plots/10p_stretch_crossfiber_LT2.json", "w") as f:
    json.dump(data_to_save, f)

# Load data from JSON file
with open("../Plots/10p_stretch_crossfiber_LT2.json", "r") as f:
...
# Save the figure
plt.savefig('../Plots/10p_stretch_crossfiber_LT2.png', dpi=300, bbox_inches='tight')
plt.close()

print("Plot saved as 10p_stretch_crossfiber_LT2.png")