...
###########################################################################################################################################################################
F_g_tot_function_LT2, F_g_tot_expression_LT2, sigma_function_LT2, F_e_LT2 = gl.LT2(mesh_LT2, F_LT2)
F_g_tot_function_KFR, F_g_tot_expression_KFR, E_e_function_KFR, E_e_expression_KFR, F_e_KFR = gl.KFR(mesh_KFR, F_KFR)
F_g_tot_function_GEG, F_g_tot_expression_GEG, E_e_function_GEG, E_e_expression_GEG, F_e_GEG = gl.GEG(mesh_GEG, F_GEG)
F_g_prev_function_GCG, F_g_prev_expression_GCG, mandel_function_GCG, F_e_GCG = gl.GCG(mesh_GCG, F_GCG)
F_g_tot_function_KOM, F_g_tot_expression_KOM, E_e_function_KOM, E_e_expression_KOM, F_e_KOM = gl.KOM(mesh_KOM, F_KOM)

J_e_LT2 = ufl.det(F_e_LT2); F_e_bar_LT2 = F_e_LT2*pow(J_e_LT2, -1/3); C_e_bar_LT2 = F_e_bar_LT2.T*F_e_bar_LT2 
J_e_KFR = ufl.det(F_e_KFR); F_e_bar_KFR = F_e_KFR*pow(J_e_KFR, -1/3); C_e_bar_KFR = F_e_bar_KFR.T*F_e_bar_KFR
J_e_GEG = ufl.det(F_e_GEG); F_e_bar_GEG = F_e_GEG*pow(J_e_GEG, -1/3); C_e_bar_GEG = F_e_bar_GEG.T*F_e_bar_GEG
J_e_GCG = ufl.det(F_e_GCG); F_e_bar_GCG = F_e_GCG*pow(J_e_GCG, -1/3); C_e_bar_GCG = F_e_bar_GCG.T*F_e_bar_GCG
J_e_KOM = ufl.det(F_e_KOM); F_e_bar_KOM = F_e_KOM*pow(J_e_KOM, -1/3); C_e_bar_KOM = F_e_bar_KOM.T*F_e_bar_KOM
...
psi_LT2 =  psi.neohookean(mu, C_e_bar_LT2) + psi.comp2(kappa, J_e_LT2); P_LT2 = ufl.diff(psi_LT2, F_e_LT2)
tensor_space_LT2 = dolfinx.fem.functionspace(mesh_LT2, basix.ufl.element(family="DG", cell=str(mesh_LT2.ufl_cell()), degree=1, shape=(3,3)))
sigma_expression_LT2 = dolfinx.fem.Expression(P_LT2*F_e_LT2.T/J_e_LT2, tensor_space_LT2.element.interpolation_points())

psi_KFR =  psi.neohookean(mu, C_e_bar_KFR) + psi.comp2(kappa, J_e_KFR); P_KFR = ufl.diff(psi_KFR, F_e_KFR)
psi_GEG =  psi.neohookean(mu, C_e_bar_GEG) + psi.comp2(kappa, J_e_GEG); P_GEG = ufl.diff(psi_GEG, F_e_GEG)
psi_GCG =  psi.neohookean(mu, C_e_bar_GCG) + psi.comp2(kappa, J_e_GCG); P_GCG = ufl.diff(psi_GCG, F_e_GCG)
tensor_space_GCG = dolfinx.fem.functionspace(mesh_GCG, basix.ufl.element(family="DG", cell=str(mesh_GCG.ufl_cell()), degree=1, shape=(3,3)))
mandel_expression_GCG = dolfinx.fem.Expression(F_e_GCG.T*P_GCG, tensor_space_GCG.element.interpolation_points())

psi_KOM =  psi.neohookean(mu, C_e_bar_KOM) + psi.comp2(kappa, J_e_KOM); P_KOM = ufl.diff(psi_KOM, F_e_KOM)

'''Create weak form'''
weak_form_LT2 = ufl.inner(ufl.grad(v_LT2), P_LT2) * ufl.dx(metadata={"quadrature_degree": 8})
weak_form_KFR = ufl.inner(ufl.grad(v_KFR), P_KFR) * ufl.dx(metadata={"quadrature_degree": 8})
weak_form_GEG = ufl.inner(ufl.grad(v_GEG), P_GEG) * ufl.dx(metadata={"quadrature_degree": 8})
weak_form_GCG = ufl.inner(ufl.grad(v_GCG), P_GCG) * ufl.dx(metadata={"quadrature_degree": 8})
weak_form_KOM = ufl.inner(ufl.grad(v_KOM), P_KOM) * ufl.dx(metadata={"quadrature_degree": 8})
...
us_LT2 = []; F_g_f_tot_LT2 = []; F_g_c_tot_LT2 = []; F_e_f_LT2 = []; F_e_c_LT2 = []
us_KFR = []; F_g_f_tot_KFR = []; F_g_c_tot_KFR = []; F_e_f_KFR = []; F_e_c_KFR = []
us_GEG = []; F_g_f_tot_GEG = []; F_g_c_tot_GEG = []; F_e_f_GEG = []; F_e_c_GEG = [] 
us_GCG = []; F_g_f_tot_GCG = []; F_g_c_tot_GCG = []; F_e_f_GCG = []; F_e_c_GCG = []
us_KOM = []; F_g_f_tot_KOM = []; F_g_c_tot_KOM = []; F_e_f_KOM = []; F_e_c_KOM = []
...
for i in range(0, N+1):
    ...
    F_g_tot_function_LT2.interpolate(F_g_tot_expression_LT2); sigma_function_LT2.interpolate(sigma_expression_LT2)                
    F_g_tot_function_KFR.interpolate(F_g_tot_expression_KFR); E_e_function_KFR.interpolate(E_e_expression_KFR)
    F_g_tot_function_GEG.interpolate(F_g_tot_expression_GEG); E_e_function_GEG.interpolate(E_e_expression_GEG)
    F_g_prev_function_GCG.interpolate(F_g_prev_expression_GCG); mandel_function_GCG.interpolate(mandel_expression_GCG)
    F_g_tot_function_KOM.interpolate(F_g_tot_expression_KOM); E_e_function_KOM.interpolate(E_e_expression_KOM)

    # Tabulate values for postprocessing
    u_LT2_new = u_LT2.copy(); us_LT2.append(u_LT2_new)
    u_KFR_new = u_KFR.copy(); us_KFR.append(u_KFR_new)
    u_GEG_new = u_GEG.copy(); us_GEG.append(u_GEG_new)
    u_GCG_new = u_GCG.copy(); us_GCG.append(u_GCG_new)
    u_KOM_new = u_KOM.copy(); us_KOM.append(u_KOM_new)

    F_g_f_tot_LT2.append(ddf.eval_expression(F_g_tot_function_LT2[0,0], mesh_LT2)[0,0]) 
    F_g_f_tot_KFR.append(ddf.eval_expression(F_g_tot_function_KFR[0,0], mesh_KFR)[0,0])
    F_g_f_tot_GEG.append(ddf.eval_expression(F_g_tot_function_GEG[0,0], mesh_GEG)[0,0])
    F_g_f_tot_GCG.append(ddf.eval_expression(F_g_prev_function_GCG[0,0], mesh_GCG)[0,0])
    F_g_f_tot_KOM.append(ddf.eval_expression(F_g_tot_function_KOM[0,0], mesh_KOM)[0,0])

    F_g_c_tot_LT2.append(ddf.eval_expression(F_g_tot_function_LT2[1,1], mesh_LT2)[0,0])
    F_g_c_tot_KFR.append(ddf.eval_expression(F_g_tot_function_KFR[1,1], mesh_KFR)[0,0])
    F_g_c_tot_GEG.append(ddf.eval_expression(F_g_tot_function_GEG[1,1], mesh_GEG)[0,0])
    F_g_c_tot_GCG.append(ddf.eval_expression(F_g_prev_function_GCG[1,1], mesh_GCG)[0,0])
    F_g_c_tot_KOM.append(ddf.eval_expression(F_g_tot_function_KOM[1,1], mesh_KOM)[0,0])
...
with open("../Plots/10p_stretch_crossfiber.json", "r") as f:
