...
J_e = ufl.det(F_e)
F_e_bar = F_e * pow(J_e, -1 / 3)
C_e_bar = F_e_bar.T * F_e_bar
...
tensor_space = dolfinx.fem.functionspace(
    mesh,
    basix.ufl.element(family="DG", cell=str(mesh.ufl_cell()), degree=1, shape=(3, 3)),
)
sigma_stress_expression = dolfinx.fem.Expression(
    P * F_e.T / J_e, tensor_space.element.interpolation_points()
)
...
for i in range(0, N + 1):
    ...
    sigma_stress_function.interpolate(
        sigma_stress_expression
    )  # Update elastic deformation tensor
...
data_to_save = {
    "data_lists": [[F_g_f_tot], [F_g_c_tot], [F_e_f], [F_e_c]],
    "titles": [
